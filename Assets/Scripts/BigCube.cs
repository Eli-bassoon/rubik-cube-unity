...
public class BigCube : MonoBehaviour
{
    ...
    public IEnumerator RotateAlongY(float angle, int rotationIndex)
    {
        ...
        if (!currentlyRotating && !PlayerSettings.SettingsOn && !PlayerSettings.GameWon && PlayerSettings.FaceRotation)
        {
            ...
            if (rotationIndex != 1)
            {
                delta = GetFaceDelta(angle, rotationIndex);
            }
            else
            {
                delta1 = GetSliceDelta(angle);
                delta2 = -delta1;
            }
            ...
            for (int x = 0; x < PlayerSettings.CubeSize; x++)
            {
                for (int z = 0; z < PlayerSettings.CubeSize; z++)
                {
                    SmallCube sc = smallCubes[x, rotationIndex, z];
                    sc.transform.parent = newRotation.transform;
                    if (rotationIndex != 1)
                    {
                        sc.number += delta;
                    }
                    ...
                    {
                        smallCubes[x, 0, z].number += delta1;
                        smallCubes[x, 2, z].number += delta2;
                    }
                }
            }
    ...
    public IEnumerator RotateAlongX(float angle, int rotationIndex)
    {
        ...
        if (!currentlyRotating && !PlayerSettings.SettingsOn && !PlayerSettings.GameWon && PlayerSettings.FaceRotation)
        {
            ...
            for (int y = 0; y < PlayerSettings.CubeSize; y++)
            {
                for (int z = 0; z < PlayerSettings.CubeSize; z++)
                {
                    SmallCube sc = smallCubes[rotationIndex, y, z];
                    sc.transform.parent = newRotation.transform;
                    if (rotationIndex != 1)
                    {
                        sc.number += delta;
                    ...
                    {
                        smallCubes[0, y, z].number += delta1;
                        smallCubes[2, y, z].number += delta2;
                    }
                }
            }
    ...
    public IEnumerator RotateAlongZ(float angle, int rotationIndex)
    {
        ...
        if (!currentlyRotating && !PlayerSettings.SettingsOn && !PlayerSettings.GameWon && PlayerSettings.FaceRotation)
        {
            ...
            for (int x = 0; x < PlayerSettings.CubeSize; x++)
            {
                for (int y = 0; y < PlayerSettings.CubeSize; y++)
                {
                    SmallCube sc = smallCubes[x, y, rotationIndex];
                    sc.transform.parent = newRotation.transform;
                    if (rotationIndex != 1)
                    {
                        sc.number += delta;
                    ...
                    {
                        smallCubes[x, y, 0].number += delta1;
                        smallCubes[x, y, 2].number += delta2;
                    }
                }
            }
    ...
    private int GetFaceDelta(float angle, int rotationIndex)
    {
        int delta = 0;
        if (rotationIndex == 0 && angle > 0) delta = -1;
        else if (rotationIndex == 0 && angle < 0) delta = +1;
        else if (rotationIndex == 2 && angle > 0) delta = +1;
        else if (rotationIndex == 2 && angle < 0) delta = -1;

        return delta;
    }

    int GetSliceDelta(float angle)
    {
        if (angle < 0)
        {
            return -1;
